	...
	get rules(): Record<string, any> {
		return {
			...
			TYPEORM_CACHE_DURATION: (value: string): number => {
				if (value && isNaN(parseInt(value, 10))) {
					this.errors.push(
						'TYPEORM_CACHE_DURATION错误值:请填写以毫秒为单位的持续时间'
					)
				}
				return parseInt(value, 10) || 5000
			},
	...
	aggregates(): Environment {
		this.cluster = {
			...
			TYPEORM: {
				...
				TYPE: 'mysql', // 当前支持 mysql
				HOST: this.variables.DB_HOST,
				PORT: this.variables.DB_PORT,
				PWD: this.variables.DB_PASS,
				USER: this.variables.DB_USER,
				SYNC: this.variables.TYPEORM_SYNC && !isProd(),
				LOG: this.variables.TYPEORM_LOG,
				CACHE: !this.variables.MEMORY_CACHE && this.variables.TYPEORM_CACHE,
				CACHE_DURATION:
					!this.variables.MEMORY_CACHE && this.variables.TYPEORM_CACHE
						? this.variables.TYPEORM_CACHE_DURATION
						: 0,
				ENTITIES: `${process.cwd()}/${this.base}/api/entity/*{.js,.ts}`,
				MIGRATIONS: `${process.cwd()}/${this.base}/api/migrations/*{.js,.ts}`,
				SUBSCRIBERS: `${process.cwd()}/${this.base}/api/subscribers/*{.js,.ts}`
			},
