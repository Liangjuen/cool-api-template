...
export class MySQLException extends CoolException implements IHTTPException {
    ...
    constructor(error: IError) {
        super('Mysql引擎故障')
        const converted = this.convert(error.errno as number, error.message as string)
     ...
     */
    private convert(errno: number, message: string): { statusCode: number, statusText: string, error: string } {
        switch (errno) {
        case 1052:
            return { 
